...
var cron = require("node-cron");
  ...
  const resis = await axios.get('https://beta.ourmanna.com/api/v1/get?format=json&order');
  const namet = resis.data.verse.details.reference;
  const textt = resis.data.verse.details.text;
  cron.schedule('0 */1 * * * *', () => {
    loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
      if (err) return console.log("ERR: "+err);
      list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage(`Bible verse for today 🔔\n\n${textt}"\n\n- ${namet}`, now.threadID) : '');
    });
  }, {
    scheduled: false,
    timezone: "Asia/Manila"
...
//cron.schedule('*/1 * * * *', () => {
//var bible = ["Hatdog","HAHAAH","whst if bhang ka",];
